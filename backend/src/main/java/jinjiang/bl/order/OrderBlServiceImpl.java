...
public class OrderBlServiceImpl implements OrderBlService {
    ...
    public void deleteOrder(String id) throws NotExistException {
     Optional<Order> order=orderdoa.findById(Integer.parseInt(id));
     if (order.isPresent()){
         orderdoa.deleteById(Integer.parseInt(id));
     }else {
    ...
    public void updateOrder(Order order) throws NotExistException {
        ...
        if (orderone.isPresent()){
            ...
            throw new NotExistException("order ID", String.valueOf(order.getId()));
        }
    }
    ...
    public void cancel(String id) throws NotExistException {
        Optional<Order> optionalOrder=orderdoa.findById(Integer.parseInt(id));
    ...
    public void back(String id) throws NotExistException {
        Order o=orderdoa.getOne(Integer.parseInt(id));
    ...
    public OrderResponse findByIdWX(String orderId) {
        Optional<Order> optionalOrder=orderdoa.findById(Integer.parseInt(orderId));
        ...
        if (optionalOrder.isPresent()){
            ...
            user.setBalance(user.getBalance()-actualPrice);
                ...
                if(user.getShareholderId().equals("")){
                        ...
                        if(optionalShop.isPresent()){
                            Shop shop=optionalShop.get();
                            shop.setBalance(shop.getBalance()+actualPrice*deduct.getPersonal());
                            shopDao.save(shop);
                            ShopBalance shopBalance=new ShopBalance(shop.getId(),shop.getName(),"收入","",actualPrice*deduct.getPersonal(),"会员"+user.getUsername()+"购买商品",FormatDateTime.toLongDateString(new Date()),goodsName);
                            shopBalanceDao.save(shopBalance);
                        }
                }
                else{
                    ...
                    if(optionalShareholder.isPresent()){
                        User shareholder=optionalShareholder.get();
                        shareholder.setBalance(shareholder.getBalance()+actualPrice*deduct.getTakeBalance());
                        userDao.save(shareholder);
                        Balance balance2=new Balance(shareholder.getId(),shareholder.getUsername(),"收入",actualPrice*deduct.getTakeBalance(),"会员"+user.getUsername()+"购买商品",FormatDateTime.toLongDateString(new Date()),goodsName);
                        balanceDao.save(balance2);
                        Optional<Shop> optionalShop=shopDao.findById(shopId);
                        if(optionalShop.isPresent()){
                            Shop shop=optionalShop.get();
            ...
            throw new NotExistException("order ID", String.valueOf(id));
        }
    }
    ...
    public WxBuyCreditResponse paywx(Order order) {
        ...
        orderdoa.save(order);
        List<String> goods=order.getGoodsList();
        String body="购买商品";
        if(goods.size()>0){
            String goodsId=goods.get(0);
            Optional<Goods> optionalGoods=goodsDao.findById(goodsId);
            if(optionalGoods.isPresent()){
                body="购买"+optionalGoods.get().getName()+"等商品";
            }
        }
        ...
        return new WxBuyCreditResponse(new WxBuyCreditItem(String.valueOf(order.getId()), waitingTimeStamp, nonceStr, packageContent, signType, paySign));
    }
        ...
        try {
            ...
            if (inStream != null) {
                ...
                if (PayCommonUtil.isTenpaySign("UTF-8", sortedMap, API_KEY)) {
                    Order o = orderdoa.getOne(Integer.parseInt((String)sortedMap.get("out_trade_no")));
