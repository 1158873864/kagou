...
public class AdminBlServiceImpl  implements AdminService {
    ...
    @Autowired
    public AdminBlServiceImpl(AdminDao admindaoone, ShopDao shopDao){
    ...
    public Admin login(String username, String password) {
        ...
        if(optionalAdmin.isPresent()){
            ...
            Optional<Shop> optionalShop=shopDao.findByName(username);
            if(optionalShop.isPresent()){
                Shop shop=optionalShop.get();
                if(shop.getPassword().equals(password)){
                    Admin admin=new Admin(shop.getName(),shop.getPassword(),"入驻商户",shop.getId(),shop.getFaceUrl());
